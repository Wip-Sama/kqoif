...
data class QoiImage(
        ...
        fun decode(bytes: ByteArray): QoiImage {
            ...
            while (pixels.size < totalPixels && offset <= bytes.size - TERMINATOR.size) {
                if (QoiOpRgb.matchTag(bytes, offset)) {
                    ...
                    val hex = (bytes[offset].toInt() and 0xFF).toString(16).padStart(2, '0').uppercase()
                    throw IllegalArgumentException("Unknown QOI chunk tag at offset $offset (byte: 0x$hex).")
                }
            }
    ...
    /**
     * Runs diagnostic analysis on this image's encoded representation.
     */
    fun analyze(): QoiDiagnosticReport = QoiDiagnostic.analyze(encode())
}
